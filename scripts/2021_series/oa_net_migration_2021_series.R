...
fpaths <- list(oa_pop  = "processed/2021_series/population_by_year/oa_population_all_EW_",
               oa_births = "processed/2021_series/births_by_year/oa_births_all_EW_",
               oa_deaths = "processed/2021_series/deaths_by_year/oa_deaths_all_EW_",
               net_mig = "processed/2021_series/net_migration_by_year/oa_net_migration_all_EW_")

dir.create("processed/2021_series/net_migration_by_year", showWarnings = FALSE)
...
for(yr in start_year:final_year){
  ...
  oa_pop <- readRDS(paste0(fpaths$oa_pop, yr, ".rds")) %>%
    lazy_dt() %>% 
    rename(value = popn)
  
  codes <- data.frame(oa_pop)$OA11CD %>% unique()
  
  comp_pop <- filter(comp_pop, OA11CD %in% codes)
  
  oa_births <- readRDS(paste0(fpaths$oa_births, yr, ".rds")) %>% 
    filter(OA11CD %in% codes)
  ...
  if(sum(is.na(net_mig))!=0){
    message(paste(sum(is.na(net_mig)), "NAs in year", yr, "- See net_mig_NAs for details."))
    net_mig_NAs <- filter(net_mig, is.na(net_mig))
    stop()
  }
